...
const STATE_FILE = path.join(process.env.HOME || "/tmp", ".claude-gemini-cli-state.json");
      ...
      if (error) {
        ...
        if (isRateLimitError(stderrText)) {
          ...
          resolve({
            ...
            type: "limit",
            retryable: false,
            message: "Gemini rate limit reached. DO NOT RETRY automatically. Wait before manual retry."
          });
        }
        // Auth error - DON'T record as failure (user needs to login)
          ...
          resolve({
            ...
            type: "auth",
            retryable: false,
            message: "Gemini CLI authentication missing or invalid. Ensure your Gemini CLI is logged in/configured."
          });
        }
        // Server error - record failure and suggest circuit breaker
          ...
          resolve({
            ...
            type: "server_error",
            retryable: false,
            message: "Gemini API server error detected. DO NOT RETRY - likely service outage. Circuit breaker will activate after multiple failures."
          });
        }
        // Timeout error
        ...
        // CLI not found
          ...
          resolve({
            ...
            type: "missing",
            retryable: false,
            message: "The `gemini` CLI is not available on PATH. Please install the official Gemini CLI."
          });
        }
        // Other errors - record failure
          ...
          resolve({
            ...
            type: "error",
            retryable: false,
            message: "Gemini CLI error. DO NOT RETRY. Stderr:\n" + stderrText.trim()
          });
        }
      } else {
    ...
    child.on("error", (err) => {
      if (err.code === "ENOENT") {
        resolve({
          ...
          type: "missing",
          retryable: false,
          message: "The `gemini` CLI is not available on PATH."
        });
      }
    });
  });
